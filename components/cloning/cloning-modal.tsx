...
import {
	...
	simulateGatewayBP,
	simulateGatewayLR,
} from "@/lib/bio/gateway";
import { type GibsonFragment, type GibsonResult, simulateGibsonMulti } from "@/lib/bio/gibson";
...
function WarnList({ items }: { items: string[] }) {
	...
	return (
		<div style={{ ...S.warn, marginTop: "12px" }}>
			{items.map((w, i) => (
				<div key={i} style={{ marginBottom: i < items.length - 1 ? "4px" : 0 }}>
					⚠ {w}
				</div>
			))}
		</div>
...
export function CloningModal({ seq, seqName, topology }: CloningModalProps) {
	...
	const [gibsonInserts, setGibsonInserts] = useState<GibsonFragment[]>([
		{ name: "Insert 1", seq: "" },
	]);
	...
	const reCompatible = useMemo(
		() => (e1 && e2Resolved ? areCompatible(e1, e2Resolved) : null),
		[e1, e2Resolved],
	);
	...
	const currentResult =
		method === "re" ? reResult : method === "gibson" ? gibsonResult : gatewayResult;
	...
	const step2Valid = useMemo(() => {
		...
		if (method === "gibson")
			return gibsonInserts.some((f) => f.seq.replace(/[^ATGCN]/gi, "").length >= 6);
		if (method === "gateway")
			return (
				gatewaySeqA.replace(/[^ATGCN]/gi, "").length >= 50 &&
				gatewaySeqB.replace(/[^ATGCN]/gi, "").length >= 50
			);
		return false;
	...
	function reset() {
		...
		setE2Name("");
		setSameEnzyme(false);
		setReOrientation("fwd");
		setReInsert("");
		setGibsonCutPos(0);
		setGibsonInserts([{ name: "Insert 1", seq: "" }]);
	...
	const triggerBtn = (
		<button
			...
			style={{
				...
				textTransform: "uppercase",
				color: "#1a4731",
				background: open ? "rgba(26,71,49,0.06)" : "none",
			...
			onMouseLeave={(e) => {
				if (!open) (e.currentTarget as HTMLButtonElement).style.background = "none";
			}}
		>
			Clone
		</button>
					...
					<div style={S.body}>
						{method === "re" && step === 1 && (
							<REStep1
								...
								e1={e1}
								e2={e2Resolved}
								compatible={reCompatible}
								onE1Change={setE1Name}
								onE2Change={setE2Name}
								onSameEnzymeChange={(v) => {
									setSameEnzyme(v);
									if (v) setE2Name("");
								}}
						...
						{method === "re" && step === 2 && (
							<REStep2 insertRaw={reInsert} e1={e1} e2={e2Resolved} onChange={setReInsert} />
						)}
						{method === "gibson" && step === 1 && (
							<GibsonStep1 seq={seq} cutPos={gibsonCutPos} onChange={setGibsonCutPos} />
						)}
						{method === "gibson" && step === 2 && (
							<GibsonStep2 fragments={gibsonInserts} onChange={setGibsonInserts} />
						)}
						{method === "gateway" && step === 1 && (
							<GatewayStep1 reaction={gatewayReaction} onChange={setGatewayReaction} />
						)}
						{method === "gateway" && step === 2 && (
							<GatewayStep2
								...
								seqA={gatewaySeqA}
								seqB={gatewaySeqB}
								onSeqAChange={setGatewaySeqA}
								onSeqBChange={setGatewaySeqB}
							/>
						)}
						{step === 3 && (
							<PreviewStep
					...
					<div style={S.footer}>
						...
						{step < 3 ? (
							...
							<button
								type="button"
								style={
									saving || !productName.trim() || currentResult?.error
										? S.btnDisabled
										: S.btnPrimary
								}
...
function REStep1({
					...
					<optgroup label="Sites in vector">
						{cutting.map((e) => (
							<option key={e.name} value={e.name}>
								{e.name} ({e.recognition}) — {siteCounts.get(e.name)} site
								{siteCounts.get(e.name) !== 1 ? "s" : ""}
							</option>
						))}
					</optgroup>
				)}
	...
	return (
		<div>
			<p style={S.prose}>
				Select the two restriction enzymes used to cut the vector. The insert will be ligated
				between them.
			</p>
			<div style={{ ...S.row, marginBottom: "16px" }}>
				<EnzSelect value={e1Name} onChange={onE1Change} label="Left enzyme (5′)" />
				{!sameEnzyme && (
					<EnzSelect value={e2Name} onChange={onE2Change} label="Right enzyme (3′)" />
				)}
			</div>
			<label
				style={{
					...
					<div style={{ display: "flex", gap: "14px" }}>
						{(["fwd", "rev"] as const).map((o) => (
							...
							>
								...
								/>
								<span
									style={{ fontFamily: "var(--font-courier)", fontSize: "10px", color: "#5a5648" }}
								>
			...
			{e1 && e2 && (
				<div style={compatible ? S.ok : S.err}>
					<span
						style={{ fontFamily: "var(--font-courier)", fontSize: "10px", letterSpacing: "0.04em" }}
					>
						{compatible
			...
			{(e1 || e2) && (
				<div style={{ marginTop: "12px", display: "flex", gap: "8px", flexWrap: "wrap" as const }}>
					{e1 && (
						<span style={S.tag}>
							{e1.name}: {siteCounts.get(e1.name) ?? 0} site
							{(siteCounts.get(e1.name) ?? 0) !== 1 ? "s" : ""} in vector
						</span>
					)}
					{e2 && !sameEnzyme && (
						<span style={S.tag}>
							{e2.name}: {siteCounts.get(e2.name) ?? 0} site
							{(siteCounts.get(e2.name) ?? 0) !== 1 ? "s" : ""} in vector
						</span>
					)}
				</div>
			)}
...
function REStep2({
	...
	const gc = ins.length
		? ((ins.split("").filter((c) => c === "G" || c === "C").length / ins.length) * 100).toFixed(1)
		: null;
	const hasE1 = e1 && ins ? findSites(ins, e1.recognition).length > 0 : false;
	const hasE2 =
		e2 && e2.name !== e1?.name && ins ? findSites(ins, e2.recognition).length > 0 : false;

	return (
		<div>
			<p style={S.prose}>
				Paste the insert sequence <strong>without</strong> the enzyme recognition sites — just the
				insert coding sequence.
			</p>
			...
			{(hasE1 || hasE2) && (
				<div style={{ ...S.warn, marginTop: "10px" }}>
					⚠ Internal {e1?.name}
					{hasE2 ? ` / ${e2?.name}` : ""} site detected — partial digestion may occur.
				</div>
			)}
		</div>
	);
...
function GibsonStep1({
	...
	return (
		<div>
			<p style={S.prose}>
				Select the position in the vector where it will be linearized. The insert must carry 15–30
				bp homology arms matching the sequences flanking this cut.
			</p>
			...
			<input
				...
				value={cutPos}
				onChange={(e) =>
					onChange(Math.max(0, Math.min(seq.length, parseInt(e.target.value, 10) || 0)))
				}
			...
			<div style={{ ...S.ok, marginTop: "14px" }}>
				<span style={{ fontFamily: "var(--font-courier)", fontSize: "10px" }}>
					Required forward primer 5′ tail:{" "}
					{seq.slice(Math.max(0, cutPos - 20), cutPos).toUpperCase() || "—"}
					<br />
					Required reverse primer 5′ tail (RC):{" "}
					{seq.slice(cutPos, cutPos + 20).toUpperCase() || "—"}
				</span>
			</div>
		</div>
	);
...
function GibsonStep2({
	fragments,
	onChange,
}: {
	fragments: GibsonFragment[];
	onChange: (f: GibsonFragment[]) => void;
}) {
	...
	function addFrag() {
		onChange([...fragments, { name: `Insert ${fragments.length + 1}`, seq: "" }]);
	}
	function removeFrag(i: number) {
		onChange(fragments.filter((_, j) => j !== i));
	}

	return (
		<div>
			<p style={S.prose}>
				Paste each insert fragment. Fragments must overlap each other and the vector by ≥15 bp, or
				the primer tails needed will be shown in the preview.
			</p>
			{fragments.map((frag, i) => {
				const clean = frag.seq.toUpperCase().replace(/[^ATGCN]/g, "");
				const gc = clean.length
					? (
							(clean.split("").filter((c) => c === "G" || c === "C").length / clean.length) *
							100
						).toFixed(1)
					: null;
				return (
					<div
						...
						style={{
							marginBottom: "16px",
							padding: "12px",
							background: "rgba(221,216,206,0.2)",
							borderRadius: "3px",
							border: "1px solid #ddd8ce",
						}}
					>
						...
						>
							<input
								value={frag.name}
								onChange={(e) => updateFrag(i, "name", e.target.value)}
								style={{ ...S.input, width: "200px", fontSize: "10px" }}
							/>
							{fragments.length > 1 && (
								<button
									type="button"
									onClick={() => removeFrag(i)}
									style={{
										background: "none",
										border: "none",
										color: "#9a9284",
						...
						<textarea
							value={frag.seq}
							onChange={(e) => updateFrag(i, "seq", e.target.value)}
...
function GatewayStep1({
	reaction,
	onChange,
}: {
	reaction: GatewayReaction;
	onChange: (r: GatewayReaction) => void;
}) {
	return (
		<div>
			<p style={S.prose}>
				Gateway recombination uses site-specific att sites. Choose the reaction type, then provide
				the two sequences.
			</p>
			...
			<div style={{ ...S.ok }}>
				...
				{reaction === "LR" ? (
					<>
						...
						>
							attL1 (in entry clone):{" "}
							<code style={{ color: "#1c1a16" }}>{ATT_SEQUENCES.attL1}</code>
						</div>
						<div style={{ fontFamily: "var(--font-courier)", fontSize: "9px", color: "#5a5648" }}>
							attL2 (in entry clone):{" "}
							<code style={{ color: "#1c1a16" }}>{ATT_SEQUENCES.attL2}</code>
						</div>
					</>
				) : (
					<>
						...
						>
							attB1 (in PCR product):{" "}
							<code style={{ color: "#1c1a16" }}>{ATT_SEQUENCES.attB1}</code>
						</div>
						<div style={{ fontFamily: "var(--font-courier)", fontSize: "9px", color: "#5a5648" }}>
							attB2 (in PCR product):{" "}
							<code style={{ color: "#1c1a16" }}>{ATT_SEQUENCES.attB2}</code>
						</div>
					</>
				)}
			</div>
...
function GatewayStep2({
	...
	seqA: string;
	seqB: string;
	onSeqAChange: (v: string) => void;
	onSeqBChange: (v: string) => void;
}) {
	const labelA =
		reaction === "LR"
			? "Entry clone sequence (attL1…GOI…attL2)"
			: "PCR product sequence (attB1…GOI…attB2)";
	const labelB =
		reaction === "LR"
			? "Destination vector sequence (contains attR1, attR2)"
			: "Donor vector sequence (contains attP1, attP2)";
	...
	return (
		<div>
			<p style={S.prose}>
				Paste both sequences. The att sites must be present — the simulator will find and use them
				automatically.
			</p>
			<label style={S.label}>{labelA}</label>
			<textarea
				value={seqA}
				onChange={(e) => onSeqAChange(e.target.value)}
				placeholder="Paste sequence…"
				rows={5}
				style={{ ...S.textarea, marginBottom: "4px" }}
				spellCheck={false}
			/>
			{cleanA.length > 0 && (
				<div style={{ marginBottom: "14px", display: "flex", gap: "6px" }}>
					<span style={S.tag}>{formatLen(cleanA.length)}</span>
				</div>
			)}
			...
			<textarea
				value={seqB}
				onChange={(e) => onSeqBChange(e.target.value)}
				placeholder="Paste sequence…"
				rows={5}
				style={{ ...S.textarea, marginBottom: "4px" }}
				spellCheck={false}
			...
			{cleanB.length > 0 && (
				<div style={{ display: "flex", gap: "6px" }}>
					<span style={S.tag}>{formatLen(cleanB.length)}</span>
				</div>
			)}
		</div>
	);
...
function PreviewStep({
	...
	if (!result)
		return (
			<div style={{ fontFamily: "var(--font-courier)", fontSize: "11px", color: "#9a9284" }}>
				Computing…
			</div>
		);
	if (result.error)
	...
	if (method === "re") {
		...
		rows.push(
			...
			{
				label: "Left junction",
				value: r.leftJunction + (r.leftJunctionCuttable ? " ✓" : " ✗ scar"),
			},
			{
				label: "Right junction",
				value: r.rightJunction + (r.rightJunctionCuttable ? " ✓" : " ✗ scar"),
			},
		);
	...
	return (
		<div>
			...
			{missingOverlaps.length > 0 && (
				<div style={{ ...S.warn, marginBottom: "14px" }}>
					<div style={{ marginBottom: "4px", fontWeight: 600 }}>
						⚠ Homology arms missing — add these primer tails:
					</div>
					{missingOverlaps.map((m, i) => (
						<div key={i} style={{ marginBottom: "2px" }}>
							{m.fragment} ({m.side}):{" "}
							<code style={{ background: "rgba(184,147,58,0.1)", padding: "0 3px" }}>
								{m.tail || "—"}
							</code>
						</div>
					))}
			...
			<WarnList
				items={(result.warnings ?? []).filter(
					(w) => missingOverlaps.length === 0 || !w.startsWith("No "),
				)}
			/>
