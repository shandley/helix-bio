...
function ReadRow({
	...
	return (
		<div style={{ padding: "8px 12px", borderBottom: "1px solid rgba(221,216,206,0.5)" }}>
			<div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: r ? "4px" : 0 }}>
				...
				/>
				<span style={{ fontFamily: "var(--font-courier)", fontSize: "9px", color: "#1c1a16", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
					{read.name}
				</span>
				<button type="button" onClick={onRemove}
					style={{ fontFamily: "var(--font-courier)", fontSize: "9px", color: "#9a9284", background: "none", border: "none", cursor: "pointer", padding: "0 2px" }}>
					×
				</button>
			</div>
			...
			{r && (
				<div style={{ paddingLeft: "16px" }}>
					<div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
						{/* Strand + position */}
						<span style={{ fontFamily: "var(--font-courier)", fontSize: "8px", color: r.strand === "+" ? "#1a4731" : "#7c3aed" }}>
							{r.strand} {r.refStart + 1}–{r.refEnd}
						</span>
						{/* Identity */}
						<span style={{ fontFamily: "var(--font-courier)", fontSize: "8px", color: r.identity >= 0.99 ? "#1a4731" : r.identity >= 0.95 ? "#b8933a" : "#a02828" }}>
					...
					{r.mismatches.length > 0 && r.mismatches.length <= 10 && (
						<div style={{ marginTop: "3px" }}>
							{r.mismatches.map((m, i) => (
								<div key={i} style={{ fontFamily: "var(--font-courier)", fontSize: "7.5px", color: "#a02828", letterSpacing: "0.03em" }}>
					...
					{r.mismatches.length > 10 && (
						<div style={{ fontFamily: "var(--font-courier)", fontSize: "7.5px", color: "#a02828", marginTop: "2px" }}>
...
export function AlignPanel({ seq, topology, onAlignmentResults }: AlignPanelProps) {
	...
	const addPasted = useCallback(() => {
		...
		if (parsed.length === 0) { setError("No valid sequence found"); return; }
	...
	}, [reads, seq, topology, onAlignmentResults]);
	...
	const btnStyle: React.CSSProperties = {
		fontFamily: "var(--font-courier)", fontSize: "9px", letterSpacing: "0.08em",
	...
	return (
		<div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
			...
			<div style={{ flexShrink: 0, padding: "10px 12px", borderBottom: "1px solid #ddd8ce" }}>
				...
				>
					<span style={{ fontFamily: "var(--font-courier)", fontSize: "9px", color: "#9a9284", letterSpacing: "0.04em" }}>
				...
				<div style={{ display: "flex", gap: "6px" }}>
					<textarea
						...
						rows={2}
						style={{
							flex: 1, fontFamily: "var(--font-courier)", fontSize: "9px",
							color: "#1c1a16", background: "#f5f0e8", border: "1px solid #ddd8ce",
							borderRadius: "2px", padding: "4px 6px", resize: "none", letterSpacing: "0.04em",
						}}
					/>
					<button type="button" onClick={addPasted} style={{ ...btnStyle, background: "#1a4731", color: "white", alignSelf: "flex-end" }}>
						Add
					</button>
				</div>

				{error && (
					<div style={{ fontFamily: "var(--font-courier)", fontSize: "9px", color: "#a02828", marginTop: "6px", background: "rgba(160,40,40,0.06)", border: "1px solid rgba(160,40,40,0.2)", borderRadius: "2px", padding: "4px 8px" }}>
						{error}
					</div>
				)}
			</div>
			...
			<div style={{ flex: 1, overflowY: "auto" }}>
				{reads.length === 0 ? (
					<div style={{ padding: "24px 12px", fontFamily: "var(--font-courier)", fontSize: "9px", color: "#9a9284", textAlign: "center", lineHeight: 1.8 }}>
						Add sequencing reads above,<br />then click Align.
					</div>
				) : (
					reads.map((r) => (
			...
			{/* Actions */}
			{reads.length > 0 && (
				<div style={{ flexShrink: 0, padding: "8px 12px", borderTop: "1px solid #ddd8ce", display: "flex", gap: "8px" }}>
					<button
						type="button"
						disabled={running}
						onClick={runAlignment}
						style={{ ...btnStyle, flex: 1, background: running ? "#2d7a54" : "#1a4731", color: "white", opacity: running ? 0.7 : 1 }}
					>
						{running ? "Aligning…" : `Align ${reads.length} read${reads.length !== 1 ? "s" : ""}`}
					</button>
					<button
						type="button"
						onClick={() => { setReads([]); onAlignmentResults?.([]); }}
						style={{ ...btnStyle, background: "none", border: "1px solid #ddd8ce", color: "#9a9284" }}
					>
						Clear
					</button>
				</div>
