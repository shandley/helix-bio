				...
				return (
					<g key={size}>
						...
						<text
							...
							fontFamily="monospace"
						>
							{outOfRange && size > MAX_BP ? ">" : ""}
							{outOfRange && size < MIN_BP ? "<" : ""}
							{formatBp(size)}
							{count > 1 ? ` ×${count}` : ""}
						</text>
					</g>
			...
			>
				{(["all", "none"] as const).map((action) => (
					<button
						type="button"
						key={action}
						onClick={action === "all" ? onSelectAll : onSelectNone}
						style={{
							fontFamily: "var(--font-courier)",
							fontSize: "8px",
							letterSpacing: "0.1em",
			...
			<div style={{ flex: 1, overflowY: "auto" }}>
				{result.error ? (
					...
					<>
						...
						>
							{result.fragments.length} fragment{result.fragments.length !== 1 ? "s" : ""} ·{" "}
							{result.cutSites.length} cut{result.cutSites.length !== 1 ? "s" : ""}
						</div>
