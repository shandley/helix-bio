...
import { isValidQuery, type SearchMatch, searchSequence } from "@/lib/bio/search";
...
export function SearchPanel({ seq, topology, onMatches }: SearchPanelProps) {
	...
	const runSearch = useCallback(
		(q: string) => {
			...
			if (!isValidQuery(q)) {
				setError(
					"Invalid characters. Use A/C/G/T or IUPAC codes (R, Y, S, W, K, M, B, D, H, V, N).",
				);
				setMatches([]);
				onMatches([]);
				return;
					...
					<>
						...
						{matches.map((m, i) => (
							<div
								...
								onMouseEnter={(e) => {
									if (i !== focusIdx)
										(e.currentTarget as HTMLDivElement).style.background = "rgba(26,71,49,0.03)";
								}}
								onMouseLeave={(e) => {
									if (i !== focusIdx)
